...
$syllable = new \Smartcrawl_Vendor\Vanderlee\Syllable\Syllable($language);
...
foreach ($languages as $value => $name) {
    ...
    echo $value;
    ?>" <?php 
    echo $value == $language ? 'selected="selected"' : '';
    ?>><?php 
    echo $name;
    ?></option>
                <?php 
...
?>
    ...
    <div>
        <label>
            <textarea name="source" cols="80" rows="10"><?php 
echo $source;
?></textarea>
        </label>
    </div>
...
echo \nl2br($syllable->hyphenateText($source));
?>
</div>
...
<div class="example">
    ...
    <h5>&amp;#8203; entities</h5>
    <?php 
$syllable->setHyphen(new \Smartcrawl_Vendor\Vanderlee\Syllable\Hyphen\ZeroWidthSpace());
echo \nl2br($syllable->hyphenateText($source));
?>
</div>
...
<div class="example">
    ...
    <?php 
$syllable->setHyphen(new \Smartcrawl_Vendor\Vanderlee\Syllable\Hyphen\Dash());
echo \nl2br($syllable->hyphenateText($source));
?>
</div>
</body>
