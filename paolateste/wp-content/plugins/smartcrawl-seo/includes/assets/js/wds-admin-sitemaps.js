...
(function ($, undefined) {
	...
	if (newsSitemapTab) {
		render(<ErrorBoundary><NewsSitemapTab
			newsSitemapUrl={Config_Values.get('news_sitemap_url', 'news')}
	...
	const sitemapTroubleshooting = document.getElementById('wds-troubleshooting-sitemap-placeholder');
	if (sitemapTroubleshooting) {
		render(
			<React.Fragment>
				<ErrorBoundary><SitemapTroubleshoot
					nonce={Config_Values.get('nonce', 'sitemaps')}
					sitemapUrl={Config_Values.get('sitemap_url', 'sitemaps')}
				/></ErrorBoundary>
				<div/>
			</React.Fragment>,
			sitemapTroubleshooting
		)
	}
