...
import {isNonEmpty, isUrlValid, isValuePlainText} from "../utils/validators";
import Config_Values from "../../es6/config-values";
import Button from "../button";

const DestinationField = fieldWithValidation(TextInputField, [isNonEmpty, isValuePlainText, isUrlValid]);
	...
	render() {
		...
		return <Modal id="wds-issue-redirect"
			...
			<div style={{display: "flex", justifyContent: "space-between"}}>
				...
				<Button id="wds-apply-crawl-item-redirect"
						text={__('Apply', 'wds')}
						onClick={onSubmit}
						icon="sui-icon-check"
