...
export default class CrawlReport extends React.Component {
			...
			<div className="sui-accordion wds-draw-left">
				{types.map(type => {
					...
					const renderControls = type === 'sitemap'
						? (type, activeTab, count) => this.renderSitemapControls(type, activeTab, count)
						: (type, activeTab, count) => this.renderControls(type, activeTab, count);
	...
	getIgnoreAllOfTypeButton(type, count) {
		return <Button id={'wds-ignore-all-' + type}
	...
	getIgnoreAllButton() {
		...
		return <IgnoreAllButton id="wds-crawler-ignore-all"
	...
	getRestoreAllButton(type, count) {
		return <Button id={'wds-restore-all-' + type}
					   icon="sui-icon-plus"
					   text={count > 1 ? __('Restore All', 'wds') : __('Restore', 'wds')}
					   ghost={true}
					   loading={this.state.requestInProgress === type}
					   disabled={this.state.requestInProgress}
	...
	renderSitemapControls(type, activeTab, count) {
		...
		return <React.Fragment>
			...
			<Button id="wds-add-all-to-sitemap"
					icon="sui-icon-plus"
