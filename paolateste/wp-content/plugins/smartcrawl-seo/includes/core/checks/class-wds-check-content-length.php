...
class Smartcrawl_Check_Content_Length extends Smartcrawl_Check_Abstract {
	...
	public function get_status_msg() {
		...
		return false === $this->state
			...
			? sprintf( __( 'The text contains %1$d words which is less than the recommended minimum of %2$d words', 'wds' ), $this->wordcount, $this->get_min() )
			// translators: %s Word count.
			: sprintf( __( 'The text contains %1$d words which is more than the recommended minimum of %2$d words', 'wds' ), $this->wordcount, $this->get_min() );
	}
