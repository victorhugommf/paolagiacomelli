        ...
        <div class="testimonials_item">
            <div class="testimonial_item_wrapper">
                <div class="testimonials_content">'.
                    ...
                    (!empty($featured_image_url) &&  $view_type != "type1" ? '<div class="testimonials_photo"><img '.esc_attr($round_imgs).' src="' . aq_resize($featured_image_url, $img_width*2, $img_height*2, true, true, true) . '" alt="'.esc_attr($featured_image_alt).'" style="width:' . esc_attr($img_width) . 'px; height:' . esc_attr($img_height) . 'px;" /></div>' : '' ) .
                    ($view_type != "type4" ? '<div class="testimonials_title"' . $sign_styles . '>' . esc_html($tstm_author) . (!empty($tstm_author_position) ? '<div class="testimonials_author_position">'.esc_html($tstm_author_position).'</div>' : '') . '</div>' : '').
                    (!empty($featured_image_url) &&  $view_type === "type1" ? '<div class="testimonials_photo"><img '.esc_attr($round_imgs).' src="' . aq_resize($featured_image_url, $img_width*2, $img_height*2, true, true, true) . '" alt="'.esc_attr($featured_image_alt).'" style="width:' . esc_attr($img_width) . 'px; height:' . esc_attr($img_height) . 'px;" /></div>' : '' ) . '
                </div>
            </div>
