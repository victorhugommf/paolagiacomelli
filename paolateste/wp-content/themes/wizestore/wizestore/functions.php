...
require_once(get_template_directory() . "/deprecated.php");
	...
	function gt3_get_pf_type_output($pf, $width, $height, $featured_image){
        ...
        if (class_exists( 'RWMB_Loader' )) {
            ...
            /* Quote */
                ...
                $compile .= '<div class="blog_post_media blog_post_media--quote">' . (strlen($quote_author) && !empty($quote_author_image) ? '<div class="post_media_info">' . (!empty($quote_author_image) ? '<img src="'.esc_url($quote_author_image).'"  class="quote_image" alt="'.esc_attr($alt_text).'">' : '') . '<div class="quote_author">' . esc_attr($quote_author) . '</div></div>' : '') . (strlen($quote_text) ? '<div class="quote_text"><i class="blog_post_media__icon blog_post_media__icon--quote fa fa-quote-left"></i>' . esc_attr($quote_text) . '</div>' : '') . '' . (strlen($quote_author) && empty($quote_author_image) ? '<div class="post_media_info"><div class="quote_author">' . esc_attr($quote_author) . '</div></div>' : '') . '</div>';
            /* Link */
                    ...
                    echo "<div class='gt3_header_builder__login-modal_container container'>";
                        ...
                        if (!is_user_logged_in()) {
                            ...
                            if(class_exists('NextendSocialLogin', false)){
                                $social_buttons = NextendSocialLogin::renderButtonsWithContainer();
                                            ...
                                            switch ($key) {
                                                ...
                                                case 'cart':
                                                    if ( class_exists('WooCommerce') ) {
                                                        ...
                                                        ob_start();
                                                            ?>
                                                            <a class="woo_icon" href="<?php echo esc_url(wc_get_cart_url()); ?>" title="<?php esc_html_e( 'View your shopping cart', 'wizestore' ); ?>"><i class='woo_mini-count'><?php echo ((WC()->cart->cart_contents_count > 0) ?  '<span>' . esc_html( WC()->cart->cart_contents_count ) .'</span>' : '') ?></i></a>
                                                            <?php
...
function gt3_tiny_mce_before_init( $settings ) {
    ...
    $style_formats = array(
        ...
        array( 'title' => esc_html__( 'List Style', 'wizestore' ), 'items' => array(
            array( 'title' => esc_html__( 'Check', 'wizestore' ), 'selector' => 'ul', 'classes' => 'gt3_list_check'),
            array( 'title' => esc_html__( 'Check in circle', 'wizestore' ), 'selector' => 'ul', 'classes' => 'gt3_list_check_circle'),
            array( 'title' => esc_html__( 'Check in square', 'wizestore' ), 'selector' => 'ul', 'classes' => 'gt3_list_check_square'),
            array( 'title' => esc_html__( 'Angle right', 'wizestore' ), 'selector' => 'ul', 'classes' => 'gt3_list_angle_right'),
            array( 'title' => esc_html__( 'Plus', 'wizestore' ), 'selector' => 'ul', 'classes' => 'gt3_list_plus'),
            array( 'title' => esc_html__( 'Times', 'wizestore' ), 'selector' => 'ul', 'classes' => 'gt3_list_times'),
            array( 'title' => esc_html__( 'Wine', 'wizestore' ), 'selector' => 'ul', 'classes' => 'gt3_list_wine'),
            )
        ),
    );
