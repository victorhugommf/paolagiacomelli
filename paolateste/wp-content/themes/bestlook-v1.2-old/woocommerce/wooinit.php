...
function gt3_woo_header_products_sidebar_top(){
    if ( ( is_shop() || is_product_category() ) && is_active_sidebar( gt3_option('products_sidebar_top') ) ) {
...
function gt3_get_social_links() {
    ...
    $compile  = '<div class="product_share">';
        $compile .= '<a href="#">'.esc_html__('SHARE','yesstore').'</a>';
...
function gt3_get_social_url($gt3_social_data) {
    global $post;
    $text = urlencode(esc_html__('A great post: ', 'yesstore').$post->post_title);
    ...
    switch ($gt3_social_data) {
        ...
        case 'mail' :
            $subject = esc_html__('Check this!', 'yesstore');
            $body = esc_html__('See more at: ', 'yesstore');
