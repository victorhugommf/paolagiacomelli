...
class Smartcrawl_Controller_Assets extends Smartcrawl_Base_Controller {
	...
	private function is_page( $page ) {
		return $this->is_smartcrawl_page() && smartcrawl_get_array_value( $_GET, 'page' ) === $page; // phpcs:ignore
	}

	private function is_smartcrawl_page() {
		global $pagenow;
		$page = (string) smartcrawl_get_array_value( $_GET, 'page' ); // phpcs:ignore
	...
	private function register_post_list_scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'nonce'               => wp_create_nonce( 'wds-metabox-nonce' ),
				'analyse_posts_delay' => (int) apply_filters( 'wds-list-table-delay', 500 ), // phpcs:ignore
			)
		);
	}
	...
	private function register_schema_settings_page_scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'settings_updated'     => smartcrawl_get_array_value( $_GET, 'settings-updated' ), // phpcs:ignore
			)
		);
	}
