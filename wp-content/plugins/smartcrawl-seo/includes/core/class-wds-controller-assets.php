...
class Smartcrawl_Controller_Assets extends Smartcrawl_Base_Controller {
	...
	private function is_page( $page ) {
		return $this->is_smartcrawl_page()
		       && smartcrawl_get_array_value( $_GET, 'page' ) === $page;
	}

	private function is_smartcrawl_page() {
		global $pagenow;
		$page = (string) smartcrawl_get_array_value( $_GET, 'page' );

		return $pagenow === 'admin.php' && strpos( $page, 'wds_' ) === 0;
	}
	...
	private function register_advanced_tools_scripts() {
		...
		$autolinks_options = Smartcrawl_Settings::get_component_options( Smartcrawl_Settings::COMP_AUTOLINKS );
		$exclusions = isset( $autolinks_options['ignorepost'] )
			? $autolinks_options['ignorepost']
			: '';

		wp_localize_script( Smartcrawl_Controller_Assets::AUTOLINKS_PAGE_JS,
			'_wds_excluded_posts',
			array(
		...
		wp_localize_script( self::AUTOLINKS_PAGE_JS, '_wds_redirects', array(
	...
	private function register_sitemap_page_scripts() {
		...
		wp_localize_script( Smartcrawl_Controller_Assets::EMAIL_RECIPIENTS_JS,
	...
	private function register_settings_page_scripts() {
		...
		$this->register_js(
			self::THIRD_PARTY_IMPORT_JS,
			'js/build/wds-third-party-import.js',
			$import_deps
		);
	...
	private function register_email_recipients_js() {
		...
		$this->register_js( self::EMAIL_RECIPIENTS_JS, 'js/build/wds-admin-email-recipients.js', $recipient_deps );
	...
	private function register_health_settings_page_scripts() {
		...
		wp_localize_script( self::EMAIL_RECIPIENTS_JS,
			'_wds_email_recipients',
			array(
