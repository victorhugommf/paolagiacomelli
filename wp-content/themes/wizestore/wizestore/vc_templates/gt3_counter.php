<?php
	...
	if ($icon_type == 'font') {
		...
		$featured_image_alt = get_post_meta( $img_id, '_wp_attachment_image_alt', true );
	}
	...
	if ($icon_type == 'image') {
		if (strlen($featured_image_url))
			...
			if ($img_width != '') {
				...
				$imageblock_content .= '<div class="icon_container icon_proportions_' . esc_attr($image_proportions) . '"><img src="' . aq_resize($featured_image_url, $img_width*2, $img_height, true, true, true) . '" alt="'.esc_attr($featured_image_alt).'" style="width:' . esc_attr($img_width) . 'px;" /></div>';
			} else {
				$imageblock_content .= '<div class="icon_container"><img src="' . esc_url($featured_image_url) . '" alt="'.esc_attr($featured_image_alt).'" /></div>';
			}
